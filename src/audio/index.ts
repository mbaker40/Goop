...
/** One goop squelch: sine drop + noise burst. `heat` 0..1 (combo) raises pitch; `fat` scales it up. */
function squelchVoice(c: AudioContext, heat: number, fat: number): void {
  ...
  osc.type = 'sine';
  const f0 = (300 + heat * 220) * jitter * (fat > 1 ? 0.7 : 1);
  osc.frequency.setValueAtTime(f0, t);
  osc.frequency.exponentialRampToValueAtTime(Math.max(50, f0 * 0.22), t + 0.1 * fat);
  og.gain.setValueAtTime(0.16 * fat * (0.85 + Math.random() * 0.3), t);
  og.gain.exponentialRampToValueAtTime(0.001, t + 0.13 * fat);
  ...
  bp.type = 'bandpass';
  bp.frequency.value = (900 + heat * 700) * jitter;
  bp.Q.value = 1.2;
  const ng = c.createGain();
  ng.gain.setValueAtTime(0.09 * fat, t);
  ng.gain.exponentialRampToValueAtTime(0.001, t + 0.09 * fat);
  noise.connect(bp).connect(ng).connect(master!);
  noise.start(t);
  noise.stop(t + 0.12 * fat);
}
